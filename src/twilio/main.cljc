(ns twilio.main
  "Kotodama WASM entrypoint for the Twilio clean-room actor (L4) — Clojure port.
  ...
  facts (`twilio.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "twilio")
(def tier "L4")
...
(def entity-specs
  [{:entity "Message"   :plural "messages"   :id-prefix "twilio_mes"
    :fields [:fromNumber :toNumber :body :status :direction] :required [:fromNumber :toNumber]
    :coerce {}                       :refs {}}
   {:entity "Call"      :plural "calls"      :id-prefix "twilio_cal"
    :fields [:fromNumber :toNumber :status :durationSec] :required [:fromNumber :toNumber]
    :coerce {:durationSec :int}      :refs {}}
   {:entity "PhoneNumber" :plural "phonenumbers" :id-prefix "twilio_pho"
    :fields [:e164 :capabilities :friendlyName] :required [:e164 :capabilities]
    :coerce {}                       :refs {}}
   {:entity "Verification" :plural "verifications" :id-prefix "twilio_ver"
    :fields [:toNumber :channel :status] :required [:toNumber :channel]
    :coerce {}                       :refs {}}
   {:entity "Recording" :plural "recordings" :id-prefix "twilio_rec"
    :fields [:callSid :durationSec :contentRef :status] :required [:callSid :durationSec]
    :coerce {:durationSec :int}      :refs {}}
   {:entity "Conference" :plural "conferences" :id-prefix "twilio_con"
    :fields [:friendlyName :status :participantCount] :required [:friendlyName :status]
    :coerce {:participantCount :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"twilio.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "twilio-compat" :tier tier :entities entities} 200])
...
(defn start! [] :twilio-compat/ready)
